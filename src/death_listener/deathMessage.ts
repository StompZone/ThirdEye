 ...
 */
export function setupDeathListener(bot: BedrockClient, channelId: TextBasedChannel) {
    bot.on("text", (packet) => {
 ...
 */
function processDeathMessage(packet: ITextPacket, channelId: TextBasedChannel) {
    ...
    if (config.useEmbed) {
        const embed = new EmbedBuilder().setColor(config.setColor).setTitle(config.setTitle).setDescription(correctedMessage).setAuthor({ name: "‎", iconURL: config.logoURL });
        sendToChannel(channelId, { embeds: [embed] }, "Could not find the in-game channel in Discord.");
    } else {
        sendToChannel(channelId, correctedMessage, "Could not find the in-game channel in Discord.");
    }
}
